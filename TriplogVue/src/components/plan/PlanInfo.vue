...
const nights = computed(() => {
  return (
    (new Date(planStore.tempPlan.endAt) -
      new Date(planStore.tempPlan.startAt)) /
    (1000 * 60 * 60 * 24)
  );
});
    ...
    <div>
      ...
      >
        <input
          ...
          v-model="planStore.tempPlan.title"
          class="form-control"
          @blur="showTitleEdit = false"
          ref="titleInput"
          @keyup.enter="$event.target.blur()"
        />
        <span v-else>{{ planStore.tempPlan.title }}</span>
      </h1>
      <!-- 설명 -->
      <p
      ...
      >
        <textarea
          ...
          v-model="planStore.tempPlan.description"
          class="form-control"
          @blur="showDescEdit = false"
          ref="descInput"
        ></textarea>
        <span v-else>{{ planStore.tempPlan.description }}</span>
      </p>
      <!-- 기간 -->
      <p
        ...
        :class="{ 'edit-mode': planStore.isEditMode }"
      >
        {{ planStore.tempPlan.startAt }} ~ {{ planStore.tempPlan.endAt }} ({{
          nights
        }}박 {{ nights + 1 }}일)
      </p>
      <!-- 달력 모달 -->
