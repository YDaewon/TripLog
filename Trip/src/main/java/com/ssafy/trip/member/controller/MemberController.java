...
public class MemberController {
	...
	// 회원가입
	@PostMapping("/join")
	@Operation(summary = "회원가입", description = "회원가입 데이터를 Insert")
	public ResponseEntity<String> join(
			@Parameter(description = "회원가입 정보", required = true) @RequestBody MemberDto memberDto) {
		try {
	...
	public ResponseEntity<?> login(
		...
		try {
			MemberDto memberDto = memberService.loginMember(loginInfo);
			if (memberDto != null) {
				session.setAttribute("userinfo", memberDto);
				return ResponseEntity.ok(memberDto); // 로그인 성공 시 사용자 정보 반환
			} else {
	...
	public ResponseEntity<MemberDto> getMypage(HttpSession session) {
		MemberDto memberInfo = (MemberDto) session.getAttribute("userinfo");
		if (memberInfo != null) {
			return ResponseEntity.ok(memberInfo); // 로그인된 사용자 정보 반환
		} else {
	...
	public ResponseEntity<MemberDto> getModifyPage(HttpSession session) {
		MemberDto memberInfo = (MemberDto) session.getAttribute("userinfo");
		if (memberInfo != null) {
			return ResponseEntity.ok(memberInfo); // 로그인된 사용자 정보 반환
		} else {
	...
	public ResponseEntity<String> modify(@RequestBody MemberDto memberDto, HttpSession session) {
		try {
			memberService.updateMember(memberDto);
			session.setAttribute("userinfo", memberDto); // 세션 정보도 업데이트
	...
	@Operation(summary = "계정 삭제", description = "userId에 해당하는 계정 정보를 삭제")
